...
REPORT_HOOK_MODS_BOT = 796310311376257054
REPORT_HOOK_ADMINS_BOT = 792775700607270933
...
@bot.command(description="plots the historical usage of servers. specify the hours in the past, default 48 hours.",
             help="[hours]",
             brief="Admin Mindustry Utility")
@commands.has_any_role("Admin (Discord)", "Admin (Mindustry)")
@commands.check(is_valid_guild)
async def plotmindushistory(ctx, hours: int = 48): 
...
async def on_message(message: discord.Message):
    ...
    elif prefix == "w?" and message.channel.id == ADMIN_LOGS_CHANNEL and message.author.id == REPORT_HOOK_ADMINS_BOT:  # admin logs channel
        await vkick_anti_bot(message, bot, autoban_counts, melonbotmindusbans)
    elif prefix == "w?" and message.channel.id == MODERATOR_LOGS_CHANNEL and message.author.id == REPORT_HOOK_MODS_BOT:  # moderator logs channel
